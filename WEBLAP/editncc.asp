...
<%
    ...
    id = Request.QueryString("id")
        ...
        If (cint(id)<>0) Then
            ...
            cmdPrep.CommandText = "SELECT * FROM tbl_NhaCC WHERE MaNCC=?"
        ...
        if (NOT isnull(tenncc) and tenncc<>"" and NOT isnull(sdt) and sdt<>"" and NOT isnull(email) and email<>"" and NOT isnull(diachi) and diachi<>"") then
            ...
            if result(0) > 0 Then
                Session("Error") = "Tên nhà cung cấp hoặc email đã tồn tại trong csdl"
                ...
                if (NOT isnull(tenncc) and tenncc<>"" and NOT isnull(sdt) and sdt<>"" and NOT isnull(email) and email<>"" and NOT isnull(diachi) and diachi<>"") then
                    ...
                    cmdPrep.CommandText = "UPDATE tbl_NhaCC SET TenNCC=?,SDT=?,Email=?,DiaChi=? WHERE MaNCC=?"
                    cmdPrep.parameters.Append cmdPrep.createParameter("tenncc",202,1,255,TenNCC)
                    ...
                    Session("Success") = "Sửa nhà cung cấp thành công"
