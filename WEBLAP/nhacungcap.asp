...
' trang hien tai
    ...
    Select Case optionsearch
		Case 0
		strSQL = "SELECT COUNT(MaNCC) AS count FROM tbl_NhaCC"
		Case 1
		strSQL = "SELECT COUNT(MaNCC) AS count FROM tbl_NhaCC  Where TenNCC  Like '%"&inputsearch&"%' "
		Case 2
		strSQL = "SELECT COUNT(MaNCC) AS count FROM tbl_NhaCC  Where DiaChi  Like '%"&inputsearch&"%' "
	...
	currentUrl = "nhacungcap.asp?input-search="&inputsearch&"&option-search="&optionsearch&"&"
								...
								<div class="table-title">
									<div class="row">
										...
										<div class="col-sm-4 p-0 flex justify-content-lg-end justify-content-center">
											<a href="addncc.asp" class="btn btn-success" >											
								...
								<table class="table table-striped table-hover">
									...
									<tbody>
										...
										<%
											...
											Select Case optionsearch
												Case 0
												cmdPrep.CommandText = "SELECT * FROM tbl_NhaCC ORDER BY MaNCC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
												Case 1
												cmdPrep.CommandText = "SELECT * FROM tbl_NhaCC Where TenNCC  Like '%"&inputsearch&"%' ORDER BY MaNCC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"

												Case 2
												cmdPrep.CommandText = "SELECT * FROM tbl_NhaCC Where DiaChi Like '%"&inputsearch&"%' ORDER BY MaNCC  OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
										...
										<tr>
											...
											<td style="display:flex;">
												<a style="width: 40px;height: 40px;" href="editncc.asp?id=<%=Result("MaNCC")%>" class="btn btn-primary mr-2 "><i style="color: #fff;margin-top:8px;" class="fa-regular fa-pen-to-square"></i></a>
												<a style="cursor: pointer;" data-href="deletencc.asp?id=<%=Result("MaNCC")%>" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirm-delete" title="Delete"><i style="color:#fff;margin-top:8px;" class="fa-solid fa-trash"></i></a>
											</td>
