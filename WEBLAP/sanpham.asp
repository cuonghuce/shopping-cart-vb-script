...
' trang hien tai
	...
	Select Case optionsearch
		Case 0
		strSQL = "SELECT COUNT(MaSP) AS count FROM tbl_SanPham"
		Case 1
		strSQL = "SELECT COUNT(MaSP) AS count FROM tbl_SanPham  Where TenSP  Like '%"&inputsearch&"%' "
		Case 2
		strSQL = "SELECT COUNT(MaSP) AS count FROM tbl_SanPham  Where GiaBan  <= "&inputsearch&" "
	...
	currentUrl = "sanpham.asp?input-search="&inputsearch&"&option-search="&optionsearch&"&"
								...
								<div class="table-title">
									<div class="row">
										<div class="col-sm-8 d-flex">
											...
											<div class="d-flex col-sm-9 p-0 flex justify-content-lg-start justify-content-center" style="margin-left:30px;margin-bottom:28px">
										...
										<div class="col-sm-4 p-0 flex justify-content-lg-end justify-content-center">
											<a href="addsanpham.asp" class="btn btn-success" >											
								...
								<table class="table table-striped table-hover">
									...
									<tbody>
										...
										<%
											...
											Select Case optionsearch
												Case 0
												cmdPrep.CommandText = "SELECT * FROM tbl_SanPham ORDER BY MaSP OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
												Case 1
												cmdPrep.CommandText = "SELECT * FROM tbl_SanPham Where TenSP  Like '%"&inputsearch&"%' ORDER BY MaSP OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"

												Case 2
												cmdPrep.CommandText = "SELECT * FROM tbl_SanPham Where GiaBan <= "&inputsearch&" ORDER BY MaSP  OFFSET ? ROWS FETCH NEXT ? ROWS ONLY"
										...
										<tr>
											...
											<td style="display:flex;">
												<a style="width: 40px;height: 40px;" href="editsanpham.asp?id=<%=Result("MaSP")%>" class="btn btn-primary mr-2"><i style="color: #fff;margin-top:8px;" class="fa-regular fa-pen-to-square"></i></a>
												<a style="cursor: pointer;"  data-href="deletecategory.asp?id=<%=Result("MaSP")%>" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirm-delete" title="Delete"><i style="color:#fff;margin-top:8px;" class="fa-solid fa-trash"></i></a>
											</td>
